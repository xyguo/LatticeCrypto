...
lemma bStarFun_swap_eq_proj (B : LatticeBasis n k) (i : Fin k) (hi : i.1 + 1 < k) :
    bStarFun (swapAdjacent B i hi).basis i =
      projTrailingSubspace B.basis i (B.basis ⟨i.1 + 1, hi⟩) := by
  classical
...
lemma prefixGramDet_of_swapAdjacent_eq (B : LatticeBasis n k) (i : Fin k) (hi : i.1 + 1 < k) :
    ...
    prefixGramDet B i * (‖projTrailingSubspace B.basis i (B.basis ⟨i.1 + 1, hi⟩)‖ ^ 2 / ‖bStarFun B.basis i‖ ^ 2) := by
      field_simp;
      rw [ eq_div_iff ];
...
theorem swapAdjacent_decreases_potential_by_factor
  ...
  classical
  set r : ℝ := ‖projTrailingSubspace B.basis i (B.basis ⟨i.1 + 1, hi⟩)‖ ^ 2 /
  ...
  have hidx : (⟨i.1 + 1, hi⟩ : Fin k) = ⟨i.1 + 1, hi'⟩ := by
    apply Fin.ext
    rfl
  have hviol' : ‖projTrailingSubspace B.basis i (B.basis ⟨i.1 + 1, hi⟩)‖ ^ 2 <
      δ * ‖projTrailingSubspace B.basis i (B.basis i)‖ ^ 2 := by
...
lemma lovasz_of_firstLovaszViolation_none (B : LatticeBasis n k) (δ : ℝ)
    (h : firstLovaszViolation B δ = none) : LovaszCondition B δ := by
      ...
      have h_no_violations : ∀ i : Fin k, ¬(∃ hi : i.1 + 1 < k, δ * ‖projTrailingSubspace B.basis i (B.basis i)‖ ^ 2 > ‖projTrailingSubspace B.basis i (B.basis ⟨i.1 + 1, hi⟩)‖ ^ 2) := by
