...
lemma rhoST_mono {s₁ s₂ : ℝ} (h1 : 1 ≤ s₁) (h : s₁ ≤ s₂) (T : E ≃L[ℝ] E) (x : E) :
  ...
  have h_exp : Real.exp (-(Real.pi * (‖T x‖ / s₁) ^ 2)) ≤ Real.exp (-(Real.pi * (‖T x‖ / s₂) ^ 2)) := by
    ...
    apply div_nonneg; exact norm_nonneg _; exact le_trans (by norm_num) h1 |> le_trans <| h;
...
theorem rhoS_mono {s₁ s₂ : ℝ} (h1 : 1 ≤ s₁) (h : s₁ ≤ s₂) (x : E) :
