...
lemma fourier_transform_comp_linear_map
  ...
  have h_subst : ∀ {g : 𝔼 n → ℂ}, ∫ v, g v ∂MeasureTheory.volume = ∫ u, g (B.symm u) * (abs (B.toLinearEquiv.det : ℝ))⁻¹ ∂MeasureTheory.volume := by
    intro g;
  ...
  have h_inner : ∀ (x : 𝔼 n), ⟪B.symm x, y⟫ = ⟪x, (LinearMap.toMatrix stdBasis stdBasis B.symm).transpose.mulVec y⟫ := by
    ...
    have h_inner : ∀ (x : 𝔼 n), B.symm x = ∑ i, x i • B.symm (stdBasis i) := by
      intro x; exact (by
...
-/
theorem fourier_transform_comp_linear_map_from_lattice (L : GeometricLattice n n) (f : 𝔼 n → ℂ) (y : 𝔼 n) :
...
-/
theorem fourier_transform_comp_linear_map_from_lattice_dual (L : GeometricLattice n n) (f : 𝔼 n → ℂ) (y : 𝔼 n) :
...
noncomputable def rhoST_FT {n : ℕ+} (s : ℝ) (T : (𝔼 n) ≃L[ℝ] (𝔼 n)) (x : 𝔼 n) : ℂ :=
  𝓕 (fun v => (rhoST s T v : ℂ)) x

/-- The Fourier transform of rhoST is a scaled-tilted version of the Fourier transform of rhoS -/
theorem rhoST_FT_eq_scaled_rhoS {n : ℕ+} (s : ℝ) (T: (𝔼 n) ≃L[ℝ] (𝔼 n)) (x : 𝔼 n) :
...
-/
theorem fourierCoefficient_eq_quotient (L : GeometricLattice n n) (f : 𝔼 n → ℂ) (w : L.dual.carrier) :
...
lemma integral_shift_eq_integral_domain (f : 𝔼 n → ℝ) (L : GeometricLattice n n) (v : L.carrier) :
    ∫ x in L.basis.fundamentalDomain, |f (x + v)| = ∫ x in Set.image (fun y => (v : 𝔼 n) + y) L.basis.fundamentalDomain, |f x| := by
...
The integral of `f(x + v)` over the fundamental domain is equal to the integral of `f(x)` over the shifted fundamental domain.
-/
lemma integral_shift_eq_integral_domain_real (f : 𝔼 n → ℝ) (L : GeometricLattice n n) (v : L.carrier) :
    ∫ x in L.basis.fundamentalDomain, f (x + v) = ∫ x in Set.image (fun y => (v : 𝔼 n) + y) L.basis.fundamentalDomain, f x := by
      -- Since the function (v : 𝔼 n) + y is just a translation, which is a measure-preserving transformation, the integrals should be equal.
      have h_measure_preserving : MeasureTheory.MeasurePreserving (fun y => (v : 𝔼 n) + y) MeasureTheory.volume MeasureTheory.volume := by
    ...
    ∫ x in L.basis.fundamentalDomain, periodize f L x = ∫ x, f x := by
      ...
      have h_integral : ∑' v : L.carrier, ∫ x in Set.image (fun y => (v : 𝔼 n) + y) L.basis.fundamentalDomain, f x = ∫ x, f x := by
        ...
        · -- Since the lattice is a discrete subgroup, the addition is measurable.
          have h_measurable_vadd : Measurable (fun (p : L.carrier × 𝔼 n) => (p.1 : 𝔼 n) + p.2) := by
            fun_prop;
          constructor;
    ...
    fourierCoefficient L (fun x => periodize f L x) w = (1 / L.det : ℂ) * 𝓕 (fun v => f v) (w : 𝔼 n) := by
      ...
      have h_integral_union : ∫ x, f x * cexp (-2 * Real.pi * Complex.I * (inner ℝ x (w : 𝔼 n))) = ∑' v : L.carrier, ∫ x in Set.image (fun y => (v : 𝔼 n) + y) L.basis.fundamentalDomain, f x * cexp (-2 * Real.pi * Complex.I * (inner ℝ x (w : 𝔼 n))) := by
        ...
        · -- Since the fundamental domain is a fundamental domain for L, shifting it by different lattice points should give disjoint sets.
          have h_disjoint : ∀ v w : L.carrier, v ≠ w → Disjoint (Set.image (fun y => (v : 𝔼 n) + y) L.basis.fundamentalDomain) (Set.image (fun y => (w : 𝔼 n) + y) L.basis.fundamentalDomain) := by
            intros v w hvw;
...
-- Coe: Z^n → R^n
def zToE (z : ZVec n) : 𝔼 n := fun i => (z i : ℝ)
...
def mapToTorus : (𝔼 n) →+ UnitAddTorus (Fin n) where
...
noncomputable def quotientZnIsoUnitAddTorus : (𝔼 n) ⧸ Zn.carrier ≃+ UnitAddTorus (Fin n) := by
  ...
  -- We need to cast the domain from (𝔼 n) ⧸ f.ker to (𝔼 n) ⧸ Zn.carrier
...
-/
lemma quotientZnIsoUnitAddTorus_mk_eq_mapToTorus (x : 𝔼 n) :
...
-/
noncomputable def periodizeZnToTorus (f : 𝔼 n → ℂ) : UnitAddTorus (Fin n) → ℂ :=
...
/-
The vector `ZVec.toZnDual w` viewed as an element of `𝔼 n` is equal to `zToE w`.
-/
lemma coe_toZnDual_eq_zToE (w : Fin n → ℤ) :
    (ZVec.toZnDual w : 𝔼 n) = zToE w := by
      -- By definition of `ZVec.toZnDual`, we have `(ZVec.toZnDual w : 𝔼 n) = (ZVec.toZn w : 𝔼 n)`.
      have hZn : (ZVec.toZnDual w : 𝔼 n) = (ZVec.toZn w : 𝔼 n) := by
        congr;
...
noncomputable def torusIntegrand (f : 𝔼 n → ℂ) (w : ZVec n) (t : UnitAddTorus (Fin n)) : ℂ :=
...
-/
lemma torusFourierCoeff_torusIntegrand (f : 𝔼 n → ℂ) (w : Fin n → ℤ) :
...
-/
lemma periodizeZnToTorus_eq_periodize (f : 𝔼 n → ℂ) (x : 𝔼 n) :
...
-/
noncomputable def torusToFundamentalDomain (u : UnitAddTorus (Fin n)) : 𝔼 n :=
...
-/
lemma torusToFundamentalDomain_mapToTorus (x : 𝔼 n) (hx : x ∈ Zn.basis.fundamentalDomain) :
...
-/
theorem mFourierCoeff_eq_fourierCoefficient_for_periodization (f : 𝔼 n → ℂ) (w : Fin n → ℤ) :
...
-/
lemma continuous_periodizeQuotient {f : 𝔼 n → ℂ} (L : GeometricLattice n n)
...
-/
lemma continuous_periodizeZnToTorus {f : 𝔼 n → ℂ}
...
-/
theorem fourierSeries_Zn_eq_torus_sum (f : 𝔼 n → ℂ) (x : 𝔼 n) :
  ...
  ∑' w : Fin n → ℤ, UnitAddTorus.mFourierCoeff (periodizeZnToTorus f) w * UnitAddTorus.mFourier w (mapToTorus x) := by
      ...
      have h_bijection : Function.Bijective (fun w : Fin n → ℤ => ZVec.toZn w : (Fin n → ℤ) → Zn.carrier) := by
        ...
        · intro x;
          ...
          obtain ⟨a, ha⟩ : ∃ a : Fin n → ℤ, x = ∑ i, a i • (Zn.basis.asTopBasis i) := by
            have h_basis : ∀ x : 𝔼 n, x ∈ Zn.carrier → ∃ a : Fin n → ℤ, x = ∑ i, a i • (Zn.basis.asTopBasis i) := by
              intro x hx
    ...
    rw [ h_sum_eq ];
    exact tsum_congr fun w => by rw [ h_fourierCoefficient, h_mFourier ] ; rw [ show ( ZVec.toZnDual w : 𝔼 n ) = zToE w from by exact
      (coe_toZnDual_eq_zToE w) ] ;

/-
The Fourier series converges to the periodization.
-/
theorem fourier_series_inversion_Zn_for_periodization {f : 𝔼 n → ℂ}
  ...
  fourierSeries Zn (fourierCoefficient Zn (periodize f Zn)) x = periodize f Zn x := by
    ...
    · -- By definition of $Zn$, we know that its dual lattice is also $Zn$.
      ...
      · intro w₁ w₂ h_eq; have := congr_arg ( fun x : Zn.dual.carrier => ( x : 𝔼 n ) ) h_eq; simp_all +decide [ ZVec.toZnDual ] ;
