...
theorem smoothingParameter_eq_sInf_smoothingSet (L : GeometricLattice n n) (ε : ℝ) :
...
noncomputable abbrev _root_.LatticeCrypto.Foundations.Lattice.GeometricLattice.η (L : GeometricLattice n n) (ε : ℝ) : ℝ :=
...
lemma rhoS_inv_tendsto_one (v : 𝔼 n) :
  Filter.Tendsto (fun s => rhoS (1/s) v) (nhdsWithin 0 (Set.Ioi 0)) (nhds 1) := by
    ...
    · unfold LatticeCrypto.Foundations.Gaussian.rho; simp +decide [ mul_pow, norm_smul ] ; ring;
    · norm_num
...
-/
theorem rhoSMass_inv_tendsto_atTop (L : GeometricLattice n n) :
    Filter.Tendsto (fun s => rhoSMass (1/s) 0 L.dual) (nhdsWithin 0 (Set.Ioi 0)) Filter.atTop := by
      ...
      -- By Lemma `summable_rhoMassOn_integrand`, for any finite subset S of L.dual, the sum over S is less than or equal to the total mass.
      have h_le_total : ∀ s : ℝ, 0 < s → ∀ S : Finset L.dual.carrier, ∑ v ∈ S, rhoS (1/s) (v : 𝔼 n) ≤ rhoSMass (1 / s) 0 L.dual := by
        ...
        have h_sum_le_total : ∑ v ∈ S, rhoS (1/s) (v : 𝔼 n) ≤ ∑' v : L.dual.carrier, rhoS (1/s) (v : 𝔼 n) := by
          ...
          · have := summable_rhoMassOn_integrand ( 1 / s ) ( one_div_pos.mpr hs ) 0 L.dual ( Set.univ : Set ( 𝔼 n ) ) ; aesop;
        convert h_sum_le_total using 1;
        unfold LatticeCrypto.Foundations.Gaussian.rhoSMass;
        unfold LatticeCrypto.Foundations.Gaussian.rhoSTMass; aesop;
...
lemma tendsto_rhoS_inv_atTop_zero {n : ℕ+} (v : 𝔼 n) (hv : v ≠ 0) :
  Filter.Tendsto (fun s : ℝ => rhoS (1/s) v) Filter.atTop (nhds 0) := by
    ...
    unfold LatticeCrypto.Foundations.Gaussian.rho; norm_num [ h_norm_sq ];
  ...
  rhoS (1/s) v ≤ Real.exp (-Real.pi * (s^2 - 1) * L.succMin₁^2) * rho v := by
    ...
    have h_norm_sq : ‖v‖^2 ≥ L.succMin₁^2 := by
      have h_norm_sq_bound : ‖v‖ ≥ L.succMin₁ := by
        ...
        exact GeometricLattice.norm_ge_successiveMinima_one L v hv;
      exact pow_le_pow_left₀ ( by exact le_of_lt ( by exact
        (GeometricLattice.successiveMinima_pos L ⟨0, PNat.pos n⟩) ) ) h_norm_sq_bound 2;
...
lemma tendsto_rhoSMassOn_atTop_zero  (L : GeometricLattice n n) :
  Filter.Tendsto (fun s => rhoSMassOn (1/s) (0 : 𝔼 n) L {0}ᶜ) Filter.atTop (nhds 0) := by
    ...
    · simp +zetaDelta at *;
      intro s hs a ha; split_ifs <;> simp_all +decide [ LatticeCrypto.Foundations.Gaussian.rhoST ] ;
      rw [ abs_of_nonneg ( by exact Real.exp_nonneg _ ) ];
      field_simp;
      exact LatticeCrypto.Foundations.Gaussian.rhoS_inv_le_rho_of_ge_one _ _ hs;
    · simp +zetaDelta at *;
      intro v hv; by_cases hv' : v = 0 <;> simp_all +decide [ LatticeCrypto.Foundations.Gaussian.rhoST ] ;
...
theorem smoothingParameter_exists (L : GeometricLattice n n) (ε : ℝ) (hε : ε > 0) :
...
theorem smoothingParameter_nonneg (L : GeometricLattice n n) (ε : ℝ) :
    L.η ε ≥ 0 := by
...
theorem smoothingParameter_mono_s (L : GeometricLattice n n) (ε : ℝ) (s : ℝ) (hs : s ∈ SmoothingSet L ε) :
...
theorem smoothingParameter_mono_ε (L : GeometricLattice n n) (ε : ℝ) (hε : ε > 0) :
    ∀ ε' ≥ ε, L.η ε' ≤ L.η ε := by
  -- By definition of smoothing parameter, the set defining η(ε', L) is a superset of the set defining η(ε, L).
  intros ε' hε'
...
theorem smoothingParameter_imply (L : GeometricLattice n n) (ε : ℝ) :
...
theorem smoothingParameter_thresh (L : GeometricLattice n n) (ε : ℝ) (hε : ε > 0) :
...
-/
theorem smoothingParameter_pos (L : GeometricLattice n n) (ε : ℝ) (hε : ε > 0) :
    L.η ε > 0 := by
  -- By definition of smoothing parameter, we know that the set is non-empty.
...
variable {n : ℕ+} (L : GeometricLattice n n) (s : ℝ) (ε : ℝ)
...
theorem smoothing_parameter_ub_via_dual_succMin₁_for_ε_ge_4_pow_neg_n {n : ℕ+} (L : GeometricLattice n n) (ε : ℝ) :
  ...
  have h_gauss_tail : rhoSMassOn (1 / s) (0 : 𝔼 n) L.dual {0}ᶜ ≤ (4 : ℝ) ^ (-(n : ℝ)) := by
    ...
    convert GeometricLattice.successiveMinima_scale _ _ _ _ using 1;
    refine' div_pos ( Real.sqrt_pos.mpr <| Nat.cast_pos.mpr n.pos ) _;
    exact GeometricLattice.successiveMinima_pos L.dual ⟨0, PNat.pos n⟩;
  refine' csInf_le _ _;
...
theorem smoothing_parameter_ub_via_succMinₙ_for_ε_ge_4_pow_neg_n {n : ℕ+} (L : GeometricLattice n n) (ε : ℝ) :
  ...
  have h_transference_lb' : 1 / L.dual.succMin₁ ≤ L.succMinₙ := by
    ...
    · have := LatticeCrypto.Foundations.Gaussian.transference_lb L.dual;
      ...
      have h_dual_dual : ∀ (L : GeometricLattice n n), L.dual.dual = L := by
        intro L; exact (by
        ...
        exact GeometricLattice.eq_basis_toLattice L);
      exact h_dual_dual L;
    · exact GeometricLattice.successiveMinima_pos L.dual ⟨0, PNat.pos n⟩
...
-/
lemma rhoSMassOn_nonzero_gt_of_s_lt_inv_succMin1 (L : GeometricLattice n n) (s : ℝ) (hs : 0 < s)
    ...
    rhoSMassOn (1 / s) 0 L.dual {0}ᶜ > 2 * Real.exp (-Real.pi) := by
      have h_shortest_dual : ∃ v ∈ L.dual.nonzeroVectors, ∀ w ∈ L.dual.nonzeroVectors, ‖v‖ ≤ ‖w‖ := by
        exact GeometricLattice.exists_shortest_vector L.dual;
      ...
      have h_rho_v_minus_v : rhoSMassOn (1 / s) 0 L.dual {0}ᶜ ≥ rhoS (1 / s) v + rhoS (1 / s) (-v) := by
        ...
        · by_cases h : v = 0 <;> simp_all +decide [ Set.indicator ];
          ...
          · rw [ Finset.sum_pair ] <;> norm_num [ h ];
            · unfold LatticeCrypto.Foundations.Gaussian.rhoST; aesop;
            · exact fun h' => h <| by ext i; have := congr_fun h' i; norm_num at *; linarith;
        · norm_num +zetaDelta at *;
          intro w hw₁ hw₂ hw₃; by_cases hw₄ : w = 0 <;> simp_all +decide [ LatticeCrypto.Foundations.Gaussian.rhoST ] ;
          exact Real.exp_nonneg _;
        · exact summable_rhoMassOn_integrand ( 1 / s ) ( one_div_pos.mpr hs ) 0 L.dual { 0 } ᶜ;
      ...
      have h_s_norm_v_lt_1 : s * ‖v‖ < 1 := by
        ...
        · refine' lt_of_le_of_lt ( mul_le_mul_of_nonneg_left ( show ‖v‖ ≤ L.dual.succMin₁ from _ ) hs.le ) h_s_lt;
          have h_norm_v_le_succMin₁ : ∃ w ∈ L.dual.nonzeroVectors, ‖w‖ = L.dual.succMin₁ := by
            exact GeometricLattice.successiveMinima_attained L.dual ⟨0, PNat.pos n⟩;
...
-/
lemma rhoSMassOn_le_pow_rhoMassOn (L : GeometricLattice n n) (s : ℝ) (hs : s ≥ 1) :
    rhoSMassOn (1 / s) 0 L.dual {0}ᶜ ≤ (Real.exp (-Real.pi * L.dual.succMin₁^2))^(s^2 - 1) * rhoMassOn 0 L.dual {0}ᶜ := by
      ...
      have h_term_bound : ∀ w ∈ L.dual.nonzeroVectors, rhoS (1 / s) w ≤ rho w * (Real.exp (-Real.pi * L.dual.succMin₁ ^ 2)) ^ (s ^ 2 - 1) := by
        ...
        unfold LatticeCrypto.Foundations.Gaussian.rho; norm_num [ norm_smul, mul_pow ] ; ring;
      ...
      · convert LatticeCrypto.Foundations.Gaussian.summable_rhoMassOn_integrand ( 1 / s ) ( by positivity ) 0 L.dual { 0 } ᶜ using 1;
      · refine' Summable.mul_right _ _;
        convert summable_rhoMassOn_integrand 1 zero_lt_one 0 L.dual { 0 } ᶜ using 1
...
-/
lemma exists_s_rhoSMassOn_le (L : GeometricLattice n n) (ε : ℝ) (hε : 0 < ε) :
      ...
      have h_lim : Filter.Tendsto (fun s : ℝ => (Real.exp (-Real.pi * L.dual.succMin₁^2))^(s^2 - 1) * rhoMassOn 0 L.dual {0}ᶜ) Filter.atTop (nhds 0) := by
        have h_lim : Filter.Tendsto (fun s : ℝ => Real.exp (-Real.pi * L.dual.succMin₁^2 * (s^2 - 1))) Filter.atTop (nhds 0) := by
          ...
          exact Filter.Tendsto.const_mul_atTop ( mul_pos Real.pi_pos ( sq_pos_of_pos ( show 0 < L.dual.succMin₁ from by exact
            (GeometricLattice.successiveMinima_pos L.dual ⟨0, PNat.pos n⟩) ) ) ) ( Filter.tendsto_atTop_add_const_right _ _ ( by norm_num ) );
...
theorem smoothing_paramter_lb_via_dual_succMin₁_for_small_ε (hε : 0 < ε ∧ ε ≤ 2 * Real.exp (-Real.pi)) :
  ...
  have h_lower_bound : ∀ s > 0, s ∈ {s : ℝ | 0 < s ∧ rhoSMassOn (1 / s) (0 : (EuclideanSpace ℝ (Fin n))) L.dual {0}ᶜ ≤ ε} → 1 / L.dual.succMin₁ ≤ s := by
    ...
    exact hε.2.not_gt <| hS.2.trans_lt' <| by linarith [ Real.exp_pos ( -Real.pi ), show LatticeCrypto.Foundations.Gaussian.rhoSMassOn ( 1 / s ) 0 L.dual { 0 } ᶜ > 2 * Real.exp ( -Real.pi ) from rhoSMassOn_nonzero_gt_of_s_lt_inv_succMin1 L s hs h_contra' ] ;
    ...
    rhoMassOn (0 : 𝔼 n) L {0}ᶜ ≥ 2 * Real.exp (-Real.pi * L.succMin₁ ^ 2) := by
      ...
      have h_two_factors : rhoMassOn 0 L {0}ᶜ ≥ rho v + rho (-v) := by
        ...
        · simp +decide [ Set.indicator ];
          ...
          exact Real.exp_nonneg _;
        · convert summable_rhoMassOn_integrand 1 zero_lt_one 0 L { 0 } ᶜ using 1;
        · field_simp;
          rw [ Finset.sum_pair ] <;> norm_num [ hv₂ ];
          · unfold rhoST; norm_num [ hv₂ ] ;
...
-/
theorem rhoSMassOn_ge_exp_neg_pi_mul_s_sq_mul_succMin₁_sq {n : ℕ+} (L : GeometricLattice n n) (s : ℝ) (hs : 0 < s) :
    rhoSMassOn (1 / s) (0 : 𝔼 n) L.dual {0}ᶜ ≥ Real.exp (-Real.pi * (s * L.dual.succMin₁)^2) := by
      -- Apply `rhoSMassOn_tail_scale` to convert `rhoSMassOn (1/s)` to `rhoMassOn` of the scaled lattice `s * L.dual`.
      have h_convert : rhoSMassOn (1 / s) (0 : 𝔼 n) L.dual {0}ᶜ = rhoMassOn (0 : 𝔼 n) (L.dual.smul s hs.ne.symm) {0}ᶜ := by
        exact rhoSMassOn_nonzero_scale L.dual s hs;
      -- Apply `rhoMass_nonzero_ge_2_exp_neg_pi_mul_succMin₁_sq` to the scaled lattice `s * L.dual`.
      have h_lower_bound : rhoMassOn (0 : 𝔼 n) (L.dual.smul s hs.ne.symm) {0}ᶜ ≥ 2 * Real.exp (-Real.pi * (s * L.dual.succMin₁) ^ 2) := by
        convert rhoMass_nonzero_ge_2_exp_neg_pi_mul_succMin₁_sq _ using 2;
        -- The first successive minimum of the dual lattice of the smul of L is s times the first successive minimum of the dual lattice of L.
        have h_succMin₁_smul : (L.dual.smul s hs.ne.symm).succMin₁ = s * L.dual.succMin₁ := by
          have := GeometricLattice.successiveMinima_scale L.dual 0 s hs
          exact this;
        rw [ h_succMin₁_smul ];
      linarith [ Real.exp_pos ( -Real.pi * ( s * L.dual.succMin₁ ) ^ 2 ) ]
...
-/
lemma smoothing_parameter_lb_aux {n : ℕ+} (L : GeometricLattice n n) (ε : ℝ) (hε : ε > 0) (s : ℝ) (hs : 0 < s) :
  rhoSMass (1 / s) 0 L.dual ≤ 1 + ε → (Real.sqrt (Real.log (1 / ε) / Real.pi)) * (1 / L.dual.succMin₁) ≤ s := by
    ...
    have h_ineq : Real.exp (-Real.pi * (s * L.dual.succMin₁)^2) ≤ ε := by
      have h_ineq : rhoSMassOn (1 / s) (0 : 𝔼 n) L.dual {0}ᶜ ≥ Real.exp (-Real.pi * (s * L.dual.succMin₁)^2) := by
        exact rhoSMassOn_ge_exp_neg_pi_mul_s_sq_mul_succMin₁_sq L s hs;
    ...
    · -- Apply the lemma with the given hypotheses.
      have := this (by
      exact GeometricLattice.successiveMinima_pos L.dual ⟨0, PNat.pos n⟩) hε h_ineq;
      aesop;
...
theorem smoothing_paramter_lb_via_dual_succMin₁ (L : GeometricLattice n n) (ε : ℝ) (hε : ε > 0) :
...
theorem smoothing_paramter_lb_via_succMinₙ (L : GeometricLattice n n) (ε : ℝ) (hε : ε > 0) (hn : n ≥ Banaszczyk_transference_threshold_constant):
  ...
  have h_lb : L.succMinₙ / (2 * n) ≤ 1 / L.dual.succMin₁ := by
    ...
    · -- Since the dual of the dual of L is L itself, we can replace L.dual.dual.succMinₙ with L.succMinₙ.
      have h_dual_dual : L.dual.dual.succMinₙ = L.succMinₙ := by
        unfold LatticeCrypto.Foundations.Lattice.GeometricLattice.dual;
    ...
    filter_upwards [ Filter.eventually_ge_atTop 1 ] with n hn using
      by
        rw [
            Real.norm_of_nonneg
              ( by exact le_trans
                    ( mul_nonneg ( Real.sqrt_nonneg _ ) ( div_nonneg ( Real.sqrt_nonneg _ ) ( by exact le_of_lt ( show 0 < ( L n |> GeometricLattice.dual |> GeometricLattice.succMin₁ ) from by exact (GeometricLattice.successiveMinima_pos (L n).dual ⟨0, PNat.pos n⟩) ) ) ) )
                    ( asymptotic_tightness n |>.1 )
              ),
            Real.norm_of_nonneg
              ( by exact div_nonneg
                    ( Real.sqrt_nonneg _ )
                    ( by exact le_of_lt ( show 0 < ( L n |> GeometricLattice.dual |> GeometricLattice.succMin₁ ) from by exact (GeometricLattice.successiveMinima_pos (L n).dual ⟨0, PNat.pos n⟩) ) )
              )
           ] ;
        ...
        rw [
            Real.norm_of_nonneg
              ( div_nonneg ( Real.sqrt_nonneg _ )
                  ( le_of_lt ( show 0 < ( L n |> GeometricLattice.dual |> GeometricLattice.succMin₁ ) from by exact
                    (GeometricLattice.successiveMinima_pos (L n).dual ⟨0, PNat.pos n⟩) ) ) ),
            Real.norm_of_nonneg ( show 0 ≤ ( L n |> GeometricLattice.η ) ( 2 ^ ( - ( n : ℝ ) ) ) from by (expose_names; exact smoothingParameter_nonneg (L n) (2 ^ (-n : ℝ))) )
          ] ;
...
/-
Inequality for the exponential term based on the half-space condition.
-/
private lemma exp_ineq {n : ℕ+} (v u : 𝔼 n) (t : ℝ) (hu : ‖u‖ = 1) (h : inner ℝ u v ≥ t) (ht : t ≥ 0) :
    1 ≤ Real.exp (2 * π * inner ℝ v (t • u) - 2 * π * t^2) := by
      norm_num +zetaDelta at *;
      rw [ inner_smul_right ];
      rw [ real_inner_comm ] ; gcongr ; nlinarith [ Real.pi_pos ]
...
protected lemma rhoMassOn_le_shifted_rhoMass {n : ℕ+} (L : GeometricLattice n n) (u : 𝔼 n) (hu : ‖u‖ = 1) (t : ℝ) (ht : t ≥ 0) (x : 𝔼 n) :
  ...
  · convert summable_rhoMassOn_integrand 1 zero_lt_one x L ( 𝓗 u hu t ) ᶜ using 1;
  · refine' Summable.mul_left _ _;
    convert summable_rhoMassOn_integrand 1 zero_lt_one ( x - t • u ) L ( Set.univ : Set ( 𝔼 n ) ) using 1;
       ext; simp [rhoST, rhoS]
...
lemma rhoMassOn_le_sum {n : ℕ+} (L : GeometricLattice n n) {ι : Type*} [Fintype ι] (S : ι → Set (𝔼 n)) :
  ...
  simp [rhoMassOn];
  have h_union_expansion : ∀ v : L.carrier, (⋃ i, S i).indicator (rhoS 1) (v : 𝔼 n) ≤ ∑ i, (S i).indicator (rhoS 1) (v : 𝔼 n) := by
    intro v
    simp [Set.indicator];
  ...
  have h_summable : Summable (fun v : L.carrier => (⋃ i, S i).indicator (rhoS 1) (v : 𝔼 n)) ∧ ∀ i, Summable (fun v : L.carrier => (S i).indicator (rhoS 1) (v : 𝔼 n)) := by
    have h_summable : Summable (fun v : L.carrier => rhoS 1 (v : 𝔼 n)) := by
      unfold rhoS;
      convert summable_rhoMassOn_integrand 1 zero_lt_one 0 L _ using 1;
      swap;
      exact Set.univ;
      unfold rhoS; aesop;
    refine' ⟨ _, fun i => _ ⟩;
    ...
    · refine' Summable.of_nonneg_of_le ( fun v => _ ) ( fun v => _ ) h_summable;
      · by_cases hi : v.val ∈ S i <;> simp +decide [ hi, rhoS ];
        exact Real.exp_nonneg _;
      · by_cases hi : v.val ∈ S i <;> simp +decide [ hi, rhoS ];
        exact Real.exp_nonneg _;
  have h_summable : ∑' v : L.carrier, (⋃ i, S i).indicator (rhoS 1) (v : 𝔼 n) ≤ ∑' v : L.carrier, ∑ i, (S i).indicator (rhoS 1) (v : 𝔼 n) := by
    apply_rules [ Summable.tsum_le_tsum ];
    · exact h_summable.1;
    · exact summable_sum fun i _ => h_summable.2 i;
  have h_fubini : ∑' v : L.carrier, ∑ i, (S i).indicator (rhoS 1) (v : 𝔼 n) = ∑ i, ∑' v : L.carrier, (S i).indicator (rhoS 1) (v : 𝔼 n) := by
...
private lemma rhoMass_le_sum_halfspaces {n : ℕ+} (L : GeometricLattice n n)
  ...
  · apply_rules [ Summable.tsum_le_tsum ];
    ...
    · convert summable_rhoMassOn_integrand 1 zero_lt_one 0 L { 0 } ᶜ using 1;
    · convert summable_rhoMassOn_integrand 1 zero_lt_one 0 L _;
      · norm_num;
      · norm_num;
...
-/
lemma rhoMass_dual_bound {n : ℕ+} (L : GeometricLattice n n) (t : ℝ) (ht : t > 0)
...
-/
lemma dual_smul_eq_smul_inv {n : ℕ+} (L : GeometricLattice n n) (c : ℝ) (hc : c ≠ 0) :
    (L.smul c hc).dual ≡ᵤ L.dual.smul (1 / c) (by simp [hc]) := by
      unfold GeometricLattice.dual;
...
-/
lemma rhoSMass_dual_bound_scaled {n : ℕ+} (L : GeometricLattice n n) (s t : ℝ) (hs : s > 0) (ht : t > 0)
    ...
    rhoSMassOn (1 / s) 0 L.dual {0}ᶜ ≤ 2 * n * Real.exp (-Real.pi * t^2) * rhoSMass (1 / s) 0 L.dual := by
      ...
      · congr! 1;
        have h_dual_smul : (L.smul (1 / s) (by positivity)).dual ≡ᵤ L.dual.smul s (by positivity) := by
          convert dual_smul_eq_smul_inv L ( 1 / s ) ( by positivity ) using 1;
          norm_num;
        have h_dual_smul : ∀ (L L' : GeometricLattice n n), L ≡ᵤ L' → rhoMass 0 L = rhoMass 0 L' := by
          intros L L' h_equiv;
          unfold rhoMass;
          unfold rhoSMass;
          simp [rhoSTMass];
      ...
      · refine' le_trans _ h_lambda;
        ...
        have h_succMinₙ_smul : ∀ (L : GeometricLattice n n) (s : ℝ) (hs : s > 0), (L.smul s hs.ne').succMinₙ = s * L.succMinₙ := by
          exact fun L s hs =>
            GeometricLattice.successiveMinima_scale L ⟨↑n - 1, GeometricLattice.succMinₙ._proof_1⟩ s
              hs;
...
-- -/
theorem smoothing_parameter_ub_micciancio_regev_by_succMinₙ (L : GeometricLattice n n) (hn : n ≥ Banaszczyk_transference_threshold_constant) (ε : ℝ) (hε : ε > 0) :
